...
VIRTUAL_CHAIN_TYPE = "virtual_chain_type"
...
def validate_config(config):
    virtual_rows = config[VIRTUAL_ROWS]
    virtual_cols = config[VIRTUAL_COLS]
    if virtual_rows * virtual_cols != config[CHAIN_LENGTH]:
        raise cv.Invalid(
            f"{CHAIN_LENGTH} must equal {VIRTUAL_ROWS} * {VIRTUAL_COLS} "
            f"({virtual_rows} * {virtual_cols}) when using this component."
        )
    return config
    ...
    display.FULL_DISPLAY_SCHEMA.extend(
        {
            ...
            cv.Optional(VIRTUAL_ROWS, default=1): cv.positive_int,
            cv.Optional(VIRTUAL_COLS, default=1): cv.positive_int,
            cv.Optional(
                VIRTUAL_CHAIN_TYPE, default="CHAIN_TOP_LEFT_DOWN"
            ): VIRTUAL_CHAIN_TYPES,
        }
    ),
    validate_config,
...
async def to_code(config):
    if not config[USE_CUSTOM_LIBRARY]:
        ...
        # Pin to a specific release tag. A bare URL with version=None resolves to
        # the master branch HEAD, so a rebuild (e.g. triggered by an ESPHome
        # update) can silently pull newer VirtualMatrixPanel_T pixel-mapping code
        # and shift everything on screen. Pinning keeps the coordinate mapping
        # stable. Change the tag if you need a different known-good version.
        cg.add_library(
            "https://github.com/mrcodetastic/ESP32-HUB75-MatrixPanel-DMA#3.0.14",
            None,
        )

    cg.add_build_flag(
        f"-DMATRIX_DISPLAY_VIRTUAL_CHAIN_TYPE={config[VIRTUAL_CHAIN_TYPE]}"
    )
    ...
    cg.add(var.set_virtual_rows(config[VIRTUAL_ROWS]))
    cg.add(var.set_virtual_cols(config[VIRTUAL_COLS]))
